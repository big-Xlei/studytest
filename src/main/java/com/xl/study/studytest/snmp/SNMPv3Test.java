...
public class SNMPv3Test {
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        UsmUser user = new UsmUser(
                ...
                AuthMD5.ID, new OctetString("snmpv3@2021md5"),
                PrivDES.ID, new OctetString("snmpv3@2021des"));
        ...
        //snmp.getUSM().addUser(new OctetString("username"), new OctetString("0002651100"), user);
        snmp.getUSM().addUser(new OctetString("username"), user);

        UserTarget target = new UserTarget();
        target.setVersion(SnmpConstants.version3);
        target.setAddress(new UdpAddress("47.100.42.207/161"));
        target.setSecurityLevel(SecurityLevel.AUTH_PRIV);
        target.setSecurityName(new OctetString("username"));
        target.setTimeout(2000);    //2s
        target.setRetries(0);

        OctetString contextEngineId = new OctetString("snmpv3@2021md5");
...
//                new OID("1.3.6.1.2.1.25.4.2.1.2"),
//                new OID("1.3.6.1.2.1.25.4.2.1.7"),
//                new OID("1.3.6.1.2.1.25.5.1.1.1")
                new OID("1.3.6.1.2.1.25.2.3.1.4")
        };
        // If not null, all returned rows have an index in a range (lowerBoundIndex, upperBoundIndex]
        List<TableEvent> l = utils.getTable(target, columnOids, new OID("0"), new OID("100"));
        for (TableEvent e : l) {
